pair<int, int> getMCMF(vector<vector<int>>& adjList, vector<vector<int>>& costs, int source, int sink) {
    ...
    while (true) { // spfa
        ...
        while (!q.empty()) {
            ...
            for (int nextNode : adj[curNode]) {
                if (adjList[curNode][nextNode] - flow[curNode][nextNode] > 0 && dists[nextNode] > dists[curNode] + costs[curNode][nextNode]) {
